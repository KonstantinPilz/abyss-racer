(function (root) {
  ...
  const Codec = {
    encode(v, seq, now, epoch, ack = { b: 0, i: 0 }, extras = {}) {
      const bytes = new ArrayBuffer(600), d = new DataView(bytes); let o = 0;
    ...
    decode(bytes) {
      if (!(bytes instanceof ArrayBuffer) || bytes.byteLength > 600 || bytes.byteLength < 160) throw new Error('Invalid snapshot');
