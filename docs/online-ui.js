(function () {
  ...
  class OnlineUI {
    constructor(net) {
      ...
      root.innerHTML = '<section id="online-screen" class="online-screen" aria-label="Two or three phones"><div id="online-content"></div></section><div id="online-quality" role="status" hidden></div><div id="online-rotate" hidden>↻ Rotate for the best view</div><div id="online-pedals" hidden>' + [['brake', '◀', 'BRAKE'], ['burst', '⇧', 'BALLAST'], ['item', '?', 'ITEM'], ['throttle', '▶', 'GAS']].map(([key, icon, label]) => '<button id="online-' + key + '" class="online-pedal ' + key + '" aria-label="' + (key === 'brake' || key === 'throttle' ? 'Hold ' : 'Use ') + label.toLowerCase() + '"><b>' + icon + '</b><span>' + label + '</span></button>').join('') + '</div>';
    ...
    share() { return '<aside class="online-share"><span class="tiny-label">YOUR ROOM CODE</span><strong id="online-code"></strong><canvas id="online-qr" role="img" aria-label="Scan to join this room"></canvas>' + button('online-copy', 'Copy link') + '<small>Scan with your friends’ phone cameras,<br>or enter this code.</small><p id="online-copy-status" role="status"></p></aside>'; }
    render() {
      const n = this.n, v = n.v, s = n.state;
      let html = '';
      if (s === 'ENTRY') html = this.header('2–3 DEVICES · ONE OCEAN', 'Bring your crew.') + '<p class="online-intro">Your own screen. Your own ride.<br>Race one or two friends on phones or laptops.</p><div class="online-entry-actions">' + button('online-create', 'Create room ↗', true) + button('online-join-screen', 'Join a room') + '</div><p class="online-note">Keep every screen open. Landscape feels best.<br>Internet required · Solo and local Versus work offline.</p>';
      ...
      if (s === 'WAITING') html = this.header('YOUR OCEAN IS OPEN', 'Invite your crew.') + '<div class="online-waiting-grid">' + this.share() + '<div><div class="online-wait" aria-hidden="true"><i></i><i></i><i></i></div><h3>Waiting for your friend…</h3><p>Keep this screen open while they scan.<br>You’ll choose your rides together.</p><p class="online-note">Phone ↔ phone or phone ↔ laptop.<br>All devices need an internet connection.</p></div></div>';
      if (s === 'LOBBY') {
        const c = v.config, own = AR.VEHICLES.find(x => x.id === c.vehicles[n.index]) || AR.VEHICLES[0];
        const tally = c.vehicles.length === 3 ? v.save.data.versus.trio : null;
        const rivalry = tally ? '<p class="online-roster">TRIO RIVALRY · ' + tally.wins.map((wins, i) => 'P' + (i + 1) + ' ' + wins).join(' — ') + ' · ' + tally.matches + ' matches</p>' : '';
        const rivals = c.vehicles.map((id, i) => i === n.index ? '' : '<article class="online-rival p' + (i + 1) + '"><span class="tiny-label">RIVAL · P' + (i + 1) + '</span><h3>' + AR.VEHICLES.find(x => x.id === id).name + '</h3><p id="online-rival-ready-' + i + '"></p><p>Equal level-5 upgrades.</p></article>').join('');
        html = this.header(n.role === 'host' ? 'YOU HOST · PLAYER 1' : 'YOU JOINED · PLAYER ' + (n.index + 1), 'Ready for a rivalry?') + '<p class="online-roster">' + c.vehicles.length + ' / 3 divers · ' + (c.vehicles.length === 3 ? 'Crew complete' : 'Start with two or invite a third') + '</p>' + rivalry + '<div class="online-lobby-grid">' + (n.role === 'host' ? this.share() : '') + '<div class="online-loadout"><div class="online-rides"><article class="online-own"><span class="tiny-label">YOUR RIDE · P' + (n.index + 1) + '</span><canvas id="online-vehicle-art"></canvas><label for="online-vehicle">Vehicle</label><select id="online-vehicle">' + options(AR.VEHICLES.map(x => [x.id, x.name]), own.id) + '</select></article>' + rivals + '</div><div class="online-options">' + [['stage', 'Ocean', AR.STAGES.map(x => [x.id, x.name]), c.stage], ['mode', 'Mode', [['race', 'Race'], ['survival', 'Last Sub Standing'], ['pearl', 'Pearl Rush · 90 s'], ['arena', 'Bubble Battle · 3 hits'], ['treasure', 'Treasure Tug']], c.mode], ['target', 'Distance', [[500, '500 m'], [1000, '1,000 m'], [2000, '2,000 m']], c.target], ['carryTarget', 'Carry to win', [[20, '20 seconds'], [30, '30 seconds'], [45, '45 seconds']], c.carryTarget || 30], ['bestOf', 'Rounds', [[1, 'One round'], [3, 'Best of 3'], [5, 'Best of 5']], c.bestOf]].map(([key, label, list, value]) => '<label>' + label + '<select id="online-option-' + key + '"' + (n.role !== 'host' || (key === 'target' && c.mode !== 'race') || (key === 'carryTarget' && c.mode !== 'treasure') ? ' disabled' : '') + '>' + options(list, value) + '</select></label>').join('') + '</div><p class="online-note">' + (c.mode === 'treasure' ? 'Carry the chest to score. Hits drop it; a 300 m gap returns it between you. ' : '') + (AR.STAGES.find(s => s.id === c.stage).how || '') + ' ' + (n.role === 'host' ? 'All divers must be ready. Share this code with your friends before starting.' : 'The host chooses the match settings.') + '</p><div class="online-ready-actions">' + button('online-ready', 'READY', true) + (n.role === 'host' ? button('online-start', c.mode === 'arena' ? 'START BATTLE ↗' : c.mode === 'treasure' ? 'START TREASURE TUG ↗' : 'START RACE ↗') : '<span>Waiting for the host to start.</span>') + '</div></div></div>';
      }
      if (s === 'RECONNECTING') html = '<div class="online-reconnecting"><div class="eyebrow">ALL ROVERS PAUSED</div><h2>Reconnecting…</h2><div class="online-wait" aria-hidden="true"><i></i><i></i><i></i></div><p id="online-reconnect-time"></p><p>Keep this screen open while we restore the race.</p>' + button('online-back', 'Leave match') + '</div>';
      ...
      if ($('online-vehicle')) {
        const stats = AR.getStats(v.config.vehicles[n.index], Object.fromEntries(AR.UPGRADES.map(u => [u.id, 5]))); if (n.index) stats.color = n.index === 2 ? '#c5a0ff' : '#56dce9'; v.renderer.thumbnail($('online-vehicle-art'), stats);
    ...
    show() {
      ...
      if (!n.active) { delete document.body.dataset.online; delete document.body.dataset.racers; for (let i = 0; i < 3; i++) $('v-hud-' + i).hidden = i === 2; return; }
      document.body.dataset.online = String(n.index || 0); document.body.dataset.racers = String(v.config.vehicles.length);
      for (let i = 0; i < 3; i++) $('v-hud-' + i).hidden = i !== n.index; document.body.dataset.versus = v.phase;
      ...
      $('online-screen').hidden = match; $('online-quality').hidden = n.state !== 'LOBBY' && !(match && v.phase === 'RUNNING');
      $('online-pedals').hidden = !match || !['RUNNING', 'COUNTDOWN'].includes(v.phase); $('online-rotate').hidden = !match || v.phase !== 'RUNNING';
      ...
      if (match && v.phase === 'PAUSED') {
        $('versus-overlay-content').innerHTML = '<div class="eyebrow">ALL ROVERS PAUSED</div><h2>Holding depth.</h2><p>Keep every screen open to stay connected.</p><div class="v-result-actions">' + button('online-resume', 'RESUME', true) + button('online-leave', 'Leave match') + '</div>';
    ...
    status() {
      ...
      if ($('online-quality')) $('online-quality').textContent = ('P' + (n.index + 1) + (n.role === 'host' ? ' · HOST' : ' · GUEST')) + '　● ' + (n.rtt === null ? 'Measuring ping…' : n.rtt + ' ms' + (n.rtt > 250 ? ' · Slow connection' : ''));
      if ($('online-ready')) { $('online-ready').textContent = v.ready[n.index] ? '✓ READY · tap to undo' : 'READY'; $('online-ready').disabled = !n.allReady(); }
      v.config.vehicles.forEach((_, i) => { if ($('online-rival-ready-' + i)) $('online-rival-ready-' + i).textContent = v.ready[i] ? '✓ Ready to dive' : 'Choosing their ride…'; });
      if ($('online-start')) $('online-start').disabled = !v.ready.every(Boolean) || !n.allReady();
      ...
      if (n.state === 'MATCH' && n.role === 'guest' && v.phase === 'COUNTDOWN') {
        ...
        if (this.countKey !== key) { this.countKey = key; $('versus-overlay-content').innerHTML = '<div class="eyebrow">' + v.stage.name.toUpperCase() + '</div><h2>Round ' + v.round + ' — Dive!</h2><div class="v-countdown">' + count + '</div><p>' + v.scoreText() + '</p>'; }
      }
