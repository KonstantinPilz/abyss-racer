  ...
  class OnlineUI {
    ...
    render() {
      ...
      if ($('online-vehicle')) {
        ...
        ['stage', 'mode', 'target', 'bestOf'].forEach(key => { $('online-option-' + key).onchange = e => n.configure(key, ['target', 'bestOf'].includes(key) ? Number(e.target.value) : e.target.value); });
      }
    ...
    status() {
      ...
      if (p && n.state === 'MATCH') {
        $('online-item').querySelector('b').textContent = p.item ? AR.VERSUS_ITEMS[p.item].icon : '?';
        $('online-item').querySelector('span').textContent = p.item ? AR.VERSUS_ITEMS[p.item].name + (p.charges > 1 ? ' ×' + p.charges : '') : 'ITEM';
        $('online-burst').querySelector('span').textContent = p.rover.cooldown > 0 ? p.rover.cooldown.toFixed(1) + ' s' : 'BALLAST';
      }
    }
  }
