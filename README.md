...
7. **Sunken City** (7,400 pearls): turquoise Atlantis, columns and arches, marked opposing/following currents, and cracked bridges. First wheel contact starts a 0.6-second crumble warning, then the bridge falls in chunks into a sloped gap. Later arrivals must climb out.
...
Pearls are worth 5, golden pearls 100, and treasure chests 75. Oxygen tanks restore 60% of maximum capacity. Pearl arcs appear over actual ramp lips. A completed flip earns 100 pearls; airtime and every 100 m add bonuses. Distance and tricks earn XP. Every level grants 75 pearls, and selected levels unlock headlight colours or propeller trails. Nineteen achievements grant additional pearl rewards.
...
Choose **Versus** beside **Let's dive in** on the title screen. Versus needs a desktop keyboard; touch devices show **“Versus needs a keyboard”**. Each player chooses a vehicle, then both confirm to start. Mouse selection and **Dive together** also work. All five vehicles and all nine stages are available regardless of solo unlocks. Every vehicle uses level 5 in all six upgrade tracks; its distinctive weight, handling, and oxygen reserve still apply.
...
| **Treasure Tug** | One chest starts between the rovers. Touch it to carry it, with −12% top speed; carry for 20/30/45 seconds (default 30) to win. Crashes, torpedoes, geysers and hard bumps from above drop it with a bounce and a one-second pickup lockout for its last carrier. A gap over 300 m returns it to the midpoint. Items remain enabled; both carry timers, chest direction and a tug-of-war bar stay visible. Results show carry time and steals. |
...
Glowing **?** crates appear approximately every 120–200 m, with extra floating crates over ramps. There is one item slot; a crate is ignored while holding an item. Each diver can use the same crate independently: it disappears from your own view for two seconds after collection, then refills. Your opponent can collect immediately, so following closely never locks them out. Pearls and oxygen remain shared. Crate cooldowns freeze when paused, survive online reconnection, and reset each round. Crates become more helpful as the current gap grows: from 20 m behind they favour Turbo Current, Torpedoes and Jellyfish Nets; at 60 m behind they give approximately 38% Turbo, 23% Torpedo, 15% Net, 15% Riptide and 8% Geyser. Geyser is also available to leaders. Smaller gaps retain the varied trailing table, while the leader gets more shields, turbo, and anchors. Every item has a distinct slot icon and synth cue, a shared toast, and visual feedback.

**Catch-up delivery:** stay at least 60 m behind for six continuous seconds with an empty item slot to receive a held **Turbo Current**, even if the leader collected all the crates. Use the normal Item control to activate it. Deliveries are at least 12 seconds apart and never replace a held item. All racers must be active; a respawn or closing the gap below 60 m resets the six-second wait. Pause freezes the timers and each round resets them. This applies to all four travelling modes in local and online Versus, using current progress from each racer's own start.
...
| **Jellyfish Net** | Halves throttle/drive torque and top speed with only mild extra drag for **4 s**, with clinging jellyfish. |
| **Geyser** | Bubbles rumble beneath the opponent for **0.3 s**, then a white, frothy column launches them with 1.6× level-5 ballast impulse, a small random pitch nudge and a speed loss. Airborne targets receive 60% impulse. Roughly two seconds of airtime; a shield blocks it. The victim sees camera shake and both players hear the geyser. |
...
| **Riptide** | Swaps the opponent's throttle and brake for **5 s**, with a persistent **CONTROLS REVERSED** banner, swirling arrow and wavy screen edges. Touch pedals keep their positions and turn purple with ⇄ icons. |
...
## Play on two or three phones (online Versus)
...
Choose **PLAY ON 2–3 PHONES** on the title screen. On one device choose **Create room**, then have one or two friends scan the QR code, open **Copy link**, or choose **Join a room** and type the four-character code. Codes use uppercase letters and digits without I, L, O, 0 or 1. QR links use `https://konstantinpilz.github.io/abyss-racer/?join=CODE`; they open a prefilled join screen and wait for one tap to unlock audio and connect. A room accepts two guests. Join before the host starts; all connected divers must be ready. A fourth device, or a new arrival after the start, gets an explanation without disturbing the race. Keep every screen open.

Each diver chooses their own vehicle. The host chooses ocean, round mode, distance, and best-of count; the guest sees those settings read-only. Changing a setting clears readiness. All divers tap **READY**, then the host taps **START RACE**. Each browser saves the result once. Two-player rooms keep the existing P1/P2 rivalry; three-player rooms keep a separate trio tally. P1 is the host, P2 the first guest, and P3 the second guest. P3 has a purple rover and tag. Competitive play leaves solo progression alone.

Each device shows one full-screen viewport, their own oxygen/distance/item HUD, and separate opponent-awareness arrows. Hold **Brake** or **Gas**, and tap **Ballast** or **Item**. Keyboard users can use either local Versus key set (A/D/W/S or arrows); Space also fires ballast. P/Escape pauses or resumes every diver, and M mutes your device. Touch pedals support multiple pointers and cancellation, account for safe areas, prevent scroll/zoom while driving, and use the dynamic viewport height. Portrait remains playable with a small rotation hint. Screen wake lock is requested where available.

The host runs the **shared 120 Hz Versus physics and match rules** for all rovers, shared pickups, items, projectiles, oxygen/respawns, scores, and rounds. The guest never steps physics or decides outcomes. It sends held controls at approximately 30 Hz; input changes also travel immediately on the event channel. Cumulative sequence counters carry every burst/item edge across lost or reordered input updates, and the host releases held guest controls after 400 ms without input. The guest interpolates a short snapshot history 100 ms behind its estimated host clock and extrapolates at most 150 ms before freezing. Rovers, wheels and projectiles interpolate; crashes/respawns snap across discontinuities.

Host snapshots run at **20 Hz** with version-4 binary payloads capped at 1,200 bytes (ordinary races remain below 600). Positions have 1 cm precision in game metres; angles use 16 bits and effect timers use deciseconds. They carry all rover poses and HUD state, phase/timer/scores, up to 12 nearby projectiles, and bounded redundant pickup/event hints, plus cannon direction, the new item timers, and up to two nearest sharks per rover, geyser columns, plankton/mud flags, nearby collapsing bridges and the complete chest state with carry timers and steals. The host simulation clock drives visible vent cycles. Complete pickup/event batches and round/results metadata also use the reliable channel. Pickups regenerate deterministically from the shared stage/round seed, and the guest retains a collected-ID ledger. Event IDs deduplicate sounds, toasts, item hits and explosions. Reconnect synchronizes the complete collected ledger, every triggered bridge and the paused chest/world before resuming.

Transport uses **PeerJS 1.5.4**, dynamically loaded from the requested cdnjs URL, with the default public PeerJS signalling broker, Google STUN, and Open Relay TURN on ports 80/443 (including TLS/TCP). Two binary data channels per guest separate snapshots/input from ordered lobby/events. The state channel uses `reliable: false`; in this PeerJS release that means **unordered, with retransmission still enabled**, so stale sequence numbers are discarded and new state is skipped under backpressure. The event channel uses `reliable: true`. See the [PeerJS API](https://peerjs.com/client/api/peer) and [1.5.4 channel configuration](https://github.com/peers/peerjs/blob/v1.5.4/lib/negotiator.ts).

Ping is an application round-trip measurement over the data channel, updated every two seconds. A connection attempt times out after 15 seconds with a retry action. Short heartbeat gaps are tolerated for five seconds, while held controls still expire after 400 ms. A dropped channel or longer silence freezes the whole room with **Reconnecting…** for up to another 15 seconds. Working data channels stay open during congestion or a signalling outage; only broken channels redial. Each disconnected guest acknowledges a restored world before the host resumes, and healthy guests keep their connections. A manual pause stays paused. A failed recovery abandons the match without adding an unfinished tally. Completed results can still be recovered and saved once.

All five modes work with two or three racers. Starting grid positions rotate, direct attacks aim at the nearest active rival, and projectiles can hit either opponent. Catch-up boosts compare against the leader; slipstream can come from either rival. Ground passes ghost independently for each pair. Survival and Bubble Battle continue until only one diver remains; tied highest scores replay a round. In three-player Treasure Tug, a 300 m spread returns the chest between the outermost rovers, and your meter compares your carry time with your strongest rival.

To reduce phone memory pressure, rendered artwork on phones shares a 24 MiB cache budget (a single larger sprite is retained until it can be replaced). Desktop rendering keeps a 96 MiB budget so split-screen cameras can reuse their tiles. Eviction explicitly releases both ImageBitmap and HTMLCanvas backing stores; hiding the page clears the caches. Unchanged viewport sizes avoid canvas reallocations. Menus contain overscroll gestures to reduce accidental pull-to-refresh, and short portrait lobbies scroll without sideways overflow. These are targeted stability fixes; an actual Safari reload on the reported hotspot has not been reproduced on physical devices.
...
Progress is a version-2 JSON object stored under **`abyss-racer-save`** in `localStorage`: pearl balance, unlocks, selected loadout, per-vehicle upgrade levels, stage × vehicle records, lifetime totals, XP, achievements, sound/cosmetic settings, and `versus: { p1Wins, p2Wins, matches }`, with an optional `versus.trio: { wins: [P1, P2, P3], matches }` for three-player results. Existing saves receive a zeroed versus tally. Completed matches update that tally once; competitive play does not award solo pearls, XP, unlocks, or records. The tally appears on versus setup. Version-1 records and legacy keys migrate safely. Unknown fields and invalid numbers are discarded; corrupt or unsupported saves fall back to defaults. If storage is blocked, gameplay continues with session-only progress and reports that saving is unavailable.
...
`node tests/run-all.cjs` runs every suite serially, including the three new
`round5-*.test.cjs` suites. Mechanics checks cover net balance, geyser warning/air
strength/shields, five-second reversed controls, every new map mechanic, chest
pickup/drop/steal/lockout/teleport/win, swept shark contacts, actual jump/brake
dodges and the per-run achievement. The existing 40-second driver now advances
world mechanics on all nine maps. Online tests exercise 20% state loss and full
reconnect without guest physics. Browser captures cover solo, split-screen and a
loopback guest at 1280×720 and 844×390 @2. See [Round 5 completion evidence](docs/ROUND5_DONE.txt)
for measured results, inspected screenshots and remaining device limitations.

## Three-phone regression checks

`tests/three-phones.test.cjs` exercises the production PeerTransport and Online controllers through a deterministic test-only PeerJS boundary: three devices, 20% state loss, independent inputs/ACKs, room admission, all five modes, per-guest stalls, repeated reconnects, timeout and exactly-once trio results. `tests/three-phones-browser.test.cjs` checks each seat's actual interface at 393×428, 393×455 and 844×390 with real touch events, zero navigation or external requests, bounded artwork caches, and Safari-style HTMLCanvas cleanup. The existing broker test still checks real two-device WebRTC. All run serially with `node tests/run-all.cjs`; no test-only transport is shipped with the game.
