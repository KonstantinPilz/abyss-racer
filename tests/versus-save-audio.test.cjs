...
for (const item of ['ink', 'torpedo', 'torpedoHit', 'net', 'siphon', 'riptide', 'shield', 'turbo', 'magnet', 'anchor']) {
  const notes = [];
...
assert.equal(cueSignatures.size, 10, 'every item and the torpedo explosion have distinct sounds');
